/* =============================================================================
   ...
   (mode: payment) and saves the card. Here, on checkout.session.completed, we:
     ...
     5) FOUNDER ALERT: text Lee that the new sub-account needs its phone number
        manually linked to LeadConnector (GHL has no API for this step - see
        OneVoice_New_Customer_Onboarding_Runbook.md STEP A). Fires every time
        provisioning succeeds so this can never silently fall through again.
   ...
   ENV: STRIPE_SECRET_KEY, STRIPE_WEBHOOK_SECRET, GHL_AGENCY_TOKEN, GHL_COMPANY_ID,
        ...
        FOUNDER_ALERT_PHONE (optional, defaults to Lee's number), FOUNDER_ALERT_FROM
        (optional, defaults to the 217 A2P number).
...
// Founder SMS alert (see step 5 above) — same phone/from used by lead-intake.js.
...
function buildWelcomeEmailHtml(v) {
  ...
  const credsNote = userExists
    ...
    : `This is a real, working password &mdash; there's no separate reset step. You can change it anytime after logging in (Settings &rarr; My Profile).`;
  ...
  <tr><td align="center" style="padding:0;">
    <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;font-family:-apple-system,'Segoe UI',Helvetica,Arial,sans-serif;">
      ...
      <tr><td style="padding:30px 22px 6px;">
        ...
        <p style="font-size:15px;line-height:1.6;color:#3d4753;margin:0 0 8px;">Your payment is confirmed and your <b>7-day free trial has started</b>. Best part: <b>your account is ready right now</b> &mdash; your AI receptionist for your first listing is already built and waiting for you.</p>
        <p style="font-size:13px;line-height:1.6;color:#8a93a3;margin:0 0 14px;">Note: our platform partner may also send you a separate, generic "Activate Your Account" system email. You can ignore that one &mdash; everything you need to log in is right here.</p>
      </td></tr>
...
// 5) founder SMS alert — GHL has no API to link a new sub-account's number to
//    LeadConnector (must be done by hand: agency > Phone Integration > Sub Account
//    Settings > this location > Link to LeadConnector). This fires on every
//    successful provision so a new customer can never silently fall through
//    without Lee knowing to go do that manual step.
async function alertFounderPhoneLink(order, prov) {
  try {
    const up = await ghlPostLoc('/contacts/upsert', {
      locationId: ORDERS_LOCATION_ID, phone: FOUNDER_ALERT_PHONE,
      firstName: 'Lee', lastName: 'Founder Alert', source: 'OneVoice founder alert',
    });
    const contactId = up.data?.contact?.id || up.data?.id || '';
    if (!contactId) return { ok: false, reason: `alert-contact upsert failed: ${up.data?.message || up.status}` };
    const msg = `OneVoice: new account "${order.company || order.name || order.email}" is provisioned and needs its phone linked. GHL: agency > Phone Integration > Sub Account Settings > this location > Link to LeadConnector. Location: ${prov.locationId}`;
    const r = await ghlPostLoc('/conversations/messages', { type: 'SMS', contactId, message: msg, fromNumber: FOUNDER_ALERT_FROM }, { version: V_CONV });
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const promoCode = (m.promo || '').trim().toLowerCase();
    const order = {
      ...
      tier, term, count, listings, plan: tier === 'pro' ? 'Pro' : 'Basic', promo: promoCode,
    ...
    // 5) founder alert: new sub-account needs its phone manually linked (see the
    //    header note + alertFounderPhoneLink above). Fires whenever provisioning
    //    actually created a location, so it can't silently fall through.
    let founderAlert = { ok: false, reason: 'skipped' };
    if (prov.provisioned && prov.locationId) {
      try { founderAlert = await alertFounderPhoneLink(order, prov); } catch (e) { founderAlert = { ok: false, reason: e.message }; }
    }
    ...
    return res.status(200).json({
      ...
      founder_alert_ok: founderAlert.ok, founder_alert_reason: founderAlert.reason || '',
    });
  } catch (err) {
...
/* =============================================================================
   ...
   "foundertest" promo (internal use only, see create-onevoice-checkout.js):
   when metadata.promo === 'foundertest', the recurring plan price is forced to
   $0/mo (setup fee is already $1 via the checkout fn) so Lee can create test
   subscriptions on the LIVE Stripe account without incurring real cost.
...
// internal test promo — keep in sync with create-onevoice-checkout.js
...
async function createTrialingSubscription(session, order) {
  ...
  const t = TERM[order.term]  || TERM.monthly;
  const n = order.count;
  const isFounderTest = (order.promo || '') === FOUNDER_TEST_CODE;
  const monthly   = p.base + p.add * (n - 1);
  const recurring = isFounderTest ? 0 : Math.round(monthly * t.months * (1 - t.off)); // term total, discount applied
  const planName  = `OneVoice ${order.plan} - ${n} listing${n > 1 ? 's' : ''} (${order.term})${isFounderTest ? ' [FOUNDER TEST]' : ''}`;
  ...
  const sub = await stripe.subscriptions.create({
    ...
    trial_period_days: TRIAL_DAYS,
    default_payment_method: pmId || undefined,
    metadata: { tier: order.tier, term: order.term, listings: String(n), username: order.username || '', source: 'onevoice-checkout', promo: order.promo || '' },
  });
  return { ok: true, subId: sub.id, priceId: price.id, recurring, trialEnd: sub.trial_end };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const order = {
      ...
      promo: (m.promo || '').trim().toLowerCase(),
      amount_today: ((s.amount_total || 0) / 100).toFixed(2),
      stripe_session_id: s.id, stripe_customer: customer,
    };
